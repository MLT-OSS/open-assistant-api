    ...
    if param_type not in ["string", "integer", "number", "boolean"]:
        raise HTTPException(
            ...
            detail={
                ...
                "message": f"Param {param_name}'s type {param_type} is not supported.",
            },
        )
...
def extract_params(
    ...
    if path_item is None:
        raise HTTPException(
            status_code=400,
            detail={"error_code": "REQUEST_VALIDATION_ERROR", "message": f"No path item found for path: {path}"},
        )
    ...
    if operation is None:
        raise HTTPException(
            ...
            detail={
                "error_code": "REQUEST_VALIDATION_ERROR",
                "message": f"No operation found for method: {method} at path: {path}",
            },
        )
